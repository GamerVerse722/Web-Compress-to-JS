...
def compression():
    ...
    original_location = os.getcwd().replace(os.sep, '/')

    with open(f"{folder}/{html_filename}", "r") as mainFile:
    ...
    for y in doc.find_all("link"):
        ...
        if "http" in style:
            ...
            with open(f"{folder}/{style}", "r") as styleFile:
                cascading_style_sheets = styleFile.read()
            Folder = SaveData(f"{folder}/{style}")
            css_content_modified = re.sub(r"url\(['\"]?([^'\")]+)['\"]?\)", replace_with_base64, cascading_style_sheets)
            head = doc.find("head")
